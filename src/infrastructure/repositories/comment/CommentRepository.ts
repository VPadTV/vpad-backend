...
export class CommentRepository implements ICommentRepository {
    ...
    async create(request: any): Promise<any> {
        return await this.db.comment.create({ data: request , select: { id: true } });
    }
    ...
    async getById(request: any):Promise<any> {
        return await this.db.comment.findUnique({ where: { id: request.id }, select: {
            text: true,
            user: { select: SimpleUser.selector },
            createdAt: true,
            updatedAt: true,
    ...
    async update(request: any): Promise<any> {
        ...
        return await this.db.comment.update({ where: { id: request.id, userId: request.user.id }, data: request, select:{text: true, updatedAt: true} });
    }
    async delete(request: any): Promise<any> {
        return  await this.db.comment.delete({ where: { id: request.id, userId: request.userId } });
