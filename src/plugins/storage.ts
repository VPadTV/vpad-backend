...
export class Storage {
	...
	async processAndResizeImage(
		...
		if (
			(meta.width && meta.width > imageType) ||
			(meta.height && meta.height > imageType)
		)
	...
	async stream(key: string): Promise<StreamResponse> {
		...
		const { ContentLength, ContentType } = await this.client
			.headObject(params)
			.promise();
