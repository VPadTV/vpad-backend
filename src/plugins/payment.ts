...
export class Payment {
	...
	private constructor() {
		...
		if (!key) throw Errors.INTERNAL_SERVER_ERROR();
		this.client = new Stripe(key);
	}

	static get() {
		if (!this.instance) this.instance = new Payment();
	...
	async createSession(data: CreateCheckout): Promise<CheckoutData> {
		...
		const fee = data.product.price
			.mul(new Decimal(process.env.FEE!))
			.toNumber();
