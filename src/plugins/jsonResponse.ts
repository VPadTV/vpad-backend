...
import { ParsedHTTPBody, parseBody } from './parseBody';
import { HttpResponse } from './responses';
import { Errors, HttpError } from './errors';
import { IncomingHttpHeaders } from 'http';

type MulterFiles = { [fieldname: string]: Express.Multer.File[] };
export type RequestFiles = { [fieldname: string]: Express.Multer.File };

const transformFiles = (multerFiles: MulterFiles) => {
    const files: RequestFiles = {};
    for (const field in multerFiles) files[field] = multerFiles[field][0];
    return files;
};
...
export function jsonResponse<T extends HttpResponse>(
    ...
    return async (req: Request, res: Response) => {
        ...
        try {
            ...
            return res
                .status(statusCode)
                .json({ ...data, token: data.token ?? req.params?.token });
        } catch (error) {
