...
export class UserUseCase {
	...
	async registerUser(request: any): Promise<unknown> {
	...
	async loginUser(request: any): Promise<unknown> {
	...
	async manageBan(request: any): Promise<unknown> {
	...
	async getUsers(request: any): Promise<unknown> {
	...
	async getUserById(request: any): Promise<unknown> {
		const user = await this.userRepository.getById(request);
		if (!user) throw Errors.NOT_FOUND();
		return user;
	}

	async updateUser(request: any): Promise<unknown> {
		if (request.username !== null && !usernameRegex().test(request.username))
			throw Errors.INVALID_USERNAME();
		if (request.nickname !== null && !usernameRegex().test(request.nickname))
			throw Errors.INVALID_NICKNAME();
		if (request.email !== null && !usernameRegex().test(request.email))
			throw Errors.INVALID_EMAIL();
		if (request.password !== null && !usernameRegex().test(request.password))
			throw Errors.INVALID_PASSWORD();
		const profilePhotoData = await this.storage.getFileData(
			request.profilePhotoUrl as unknown as FileRawUpload,
			ImageType.THUMBNAIL,
		);
	...
	async deleteUser(request: any): Promise<unknown> {
