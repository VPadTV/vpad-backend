...
export class CommentUseCase {
    ...
    async createComment(request: any): Promise<any> {
    ...
    async getComments(request: any): Promise<any> {
        ...
        const [comments, total] = await this.commentRepository.getAll(
            request
        )

        return paginate(total, request.page, offset, request.size, comments.map(comment => ({
            id: comment.id,
            text: comment.text,
    ...
    async getCommentById(request: any): Promise<any> {
    ...
    async updateComment(request: any) : Promise<any> {
