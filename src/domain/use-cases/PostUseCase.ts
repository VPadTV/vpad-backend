...
export class PostUseCase {
	...
	async createPost(request: any): Promise<unknown> {
	...
	async getPostById(request: any): Promise<unknown> {
	...
	async getAllPosts(request: any): Promise<unknown> {
		if (request.user && request.userTierId) {
			...
			const authorsCheck =
				request.creatorId && request.creatorId.trim().length > 0
					? { some: { id: request.creatorId } }
					: undefined;
			const where: any = {
			...
			const posts: unknown[] = await this.postRepository.getAll(request);
	...
	async updatePost(request: any): Promise<void> {
		if (!request.id) throw Errors.MISSING_ID();
		const tags = request.tags.length
			? parseTags(request.tags.trim())
			: undefined;
		if (!tags) throw Errors.INVALID_TAGS();
		request.nsfw = boolify(request.nsfw);
		const post = await this.postRepository.getById(request);
	...
	async deletePost(request: any): Promise<void> {
	...
	async postStreamKey(request: any): Promise<void> {
		if (request.key) {
	...
	async voteSet(request: any): Promise<void> {
		if (!request.vote && !validString(request.postId))
